...
Скрипт запускается ровно раз в 4 часа (на первой минуте каждого 4-го часа), что соответствует ограничениям API HeadHunter. Расписание:
- 00:01
- 04:01
- 08:01
- 12:01
...
> **Примечание:** Для этого используется workflow-файл `.github/workflows/check_and_update_resume.yml`, который запускается по расписанию для обновления резюме.
...
### Ошибка обновления
```
Статус: ❌ не смог обновить резюме из-за touch_limit_exceeded
...
A: Каждые 4 часа и 1 минуту через GitHub Actions (00:01, 04:01, 08:01, 12:01, 16:01, 20:01 по UTC). Это соответствует ограничению бесплатного тарифа HH.ru - на бесплатном аккаунте резюме можно обновлять не чаще, чем раз в 4 часа. Если вы попытаетесь обновить резюме чаще, HH.ru вернет ошибку.
