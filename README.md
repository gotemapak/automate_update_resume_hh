...
> **Примечание:** Для этого используется workflow-файл `.github/workflows/check_and_update_resume.yml`, который запускается по расписанию и проверяет необходимость обновления резюме. Если условия выполнены, скрипт напрямую запускает обновление, не используя промежуточные workflow.
...
A: По умолчанию каждые 4 часа через GitHub Actions. Это соответствует ограничению бесплатного тарифа HH.ru - на бесплатном аккаунте резюме можно обновлять не чаще, чем раз в 4 часа. Если вы попытаетесь обновить резюме чаще, HH.ru вернет ошибку. Система проверяет возможность обновления каждые 30 минут и запускает процесс только если прошло не менее 4 часов и 1 минуты с последнего успешного обновления.
